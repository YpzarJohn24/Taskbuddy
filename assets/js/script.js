...
// store weekday name, month & month-of-day number
...
const addEventOnMultiElem = function (elements, event) {
...
const taskInputValidation = function (taskIsValid) {
  if (taskIsValid) {
    // if there is an existing task then the new task will be added before it
    ...
    // update a taskItem DOM selection
...
const removeTask = function () {
  ...
  // if the task is completed, the taskItem would be remove after 250ms , if deleted than taskItem remove instant
...
// toggle active class on menu when click menuBtn or dropdownLink
